...
Future<String> getResponseFromCozeAPI(String userMessage) async {
  ...
  var response = await http.post(
    ...
    headers: {
      ...
      'Accept': '*/*',
      'Authorization':
          'Bearer pat_LZAUkJZcLxBF7piilIVD7TimdS87EGI4pKEHBR0UwsdESP7ETFhfmU0rNXX1RF1j',
    },
    body: jsonEncode({
  ...
  if (response.statusCode == 200) {
    ...
    var jsonString = RegExp(r'\{.*\}').firstMatch(response.body)?.group(0);
    if (jsonString == null) {
      throw Exception('Failed to parse response from Coze API');
    }
    ...
    throw Exception('Failed to get response from Coze API');
  }
}
...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _handleSendPressed(types.PartialText message) async {
    ...
    try {
      ...
      final cozeMessage = types.TextMessage(
        author: _bot,
        createdAt: DateTime.now().millisecondsSinceEpoch,
        id: const Uuid().v4(),
        text: cozeResponse,
      ...
      setState(() {
        ...
        _messages.insert(0, cozeMessage);
      });
    } catch (e) {
      print('Failed to get response from Coze API: $e');
    }
    var cozeResponse = await getResponseFromCozeAPI(message.text);
    print('Coze response: $cozeResponse');
  }
